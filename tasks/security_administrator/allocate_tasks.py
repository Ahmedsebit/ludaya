...
from tasks.security_administrator.security import SECURITY as security
...
def assign_security_admin_task():
    users = User.query.filter_by(job_description='security_administrator').all()
    if users:
        for user in users:
            tasks = []
