...
from tasks.database_administrator.database_administrator import DATABASE_ADMINISTRAOR as database_administrator
...
def assign_database_admin_task():
    users = User.query.filter_by(job_description='database_administrator').all()
    if users:
        for user in users:
            tasks = []
