...
from tasks.network_administrator.networking import NETWORKING as networkings
...
def assign_network_admin_task():
    users = User.query.filter_by(job_description='network_administrator').all()
    if users:
        for user in users:
            tasks = []
